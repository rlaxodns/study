...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3*32, 32) # (n, 3*32) --> (n, 16) # bidirectional사용시 *2해주야함
    ...
    def forward(self, x, h0=None):
        ...
        if h0 is None:
            h0 = torch.zeros(3, x.size(0), 32).to(DEVICE) # num_layer, batch_size, hidden_size
...
def train(model, criterion, optimizer, loader):
    ...
    for x_batch, y_batch in loader:
        ...
        h0 = torch.zeros(3, x_batch.size(0), 32).to(DEVICE) 
...
def predict(model, data):
    ...
    with torch.no_grad():
        ...
        h0 = torch.zeros(3, data.size(0), 32).to(DEVICE) 
